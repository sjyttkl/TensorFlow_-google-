...
with tf.Session() as sess:
    ...
    for i in range(1001):
        ...
        sess.run(train_step,feed_dict={x:batch_xs,y:batch_ys,keep_prob:0.7}) #这里只是训练而已。
        # 记录训练集计算的参数
        summary = sess.run(merged,feed_dict={x:batch_xs,y:batch_ys,keep_prob:1.0})# 这里需要把一些
